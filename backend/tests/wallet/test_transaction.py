...
def test_transaction_exceeding_balance():
...
def test_transaction_update():
    ...
    # Update to first recipient after initial transaction
    to_first_recipient = 25
    transaction.update(sender_wallet, first_recipient, to_first_recipient)

    assert transaction.output[first_recipient] ==\
        first_amount + to_first_recipient

    assert transaction.output[sender_wallet.address] ==\
        sender_wallet.balance - first_amount - next_amount - to_first_recipient
...
def test_invalid_reward_transaction_invalid_amount():
    ...
    reward_transaction.output[miner_wallet.address] = 2001
